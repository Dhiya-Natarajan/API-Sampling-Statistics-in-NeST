...
def run_experiment(exp):
    ...
    # Contains start time, end time, and sampling interval to run respective
    # command from a source netns to destination address (in destination netns)
    ss_schedules = defaultdict(lambda: (float("inf"), float("-inf"), 0.2))
    ping_schedules = defaultdict(lambda: (float("inf"), float("-inf"), 0.2))
    ...
    for flow in exp.flows:
        ...
        (min_start, max_stop, prev_ping_interval) = ping_schedules[(src_ns, dst_ns, dst_addr)]
        ping_schedules[(src_ns, dst_ns, dst_addr)] = (
            min(min_start, start_t),
            max(max_stop, stop_t),
            min(prev_ping_interval, flow.stats_config.ping),
        )
...
def setup_tcp_flows(
    ...
    ss_interval = flow.stats_config.ss
    ...
    if dependency and len(qdisc_stats) > 0:
        ...
        for qdisc_stat in qdisc_stats:
            tc_runner = TcRunner(
                ...
                qdisc_stat["qdisc"],
                exp_end,
                interval=qdisc_stat.get("interval", 0.2),
            )
...
def _get_start_stop_time_for_ss(
    src_ns, dst_ns, dst_addr, start_t, stop_t, ss_schedules, ss_interval=0.2
):
    """
    ...
    ss_interval: float
        Sampling interval in seconds for this ss runner (default 0.2).
        When flows with different intervals share the same (src, dst, addr)
        key, the minimum interval is used to avoid data loss.
    ...
    if (src_ns, dst_ns, dst_addr) not in ss_schedules:
        ss_schedules[(src_ns, dst_ns, dst_addr)] = (start_t, stop_t, ss_interval)
    else:
        (min_start, max_stop, prev_interval) = ss_schedules[(src_ns, dst_ns, dst_addr)]
        ss_schedules[(src_ns, dst_ns, dst_addr)] = (
            min(min_start, start_t),
            max(max_stop, stop_t),
            min(prev_interval, ss_interval),
        )
