...
class TcRunner(Runner):
    ...
    def __init__(self, ns_id, dev, qdisc, run_time, interval=0.2):
        """
        ...
        run_time : num
            total time to run tc for
        interval : float
            sampling interval in seconds (default 0.2).
        """
        self.dev = dev
        self.qdisc = qdisc
    ...
    def run(self):
        ...
        """
        super().run(
            partial(run_tc, self.ns_id, TcRunner.iterator, self.dev, self.run_time, self.interval),
